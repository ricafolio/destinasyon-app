...
export default function Header() {
  ...
  return (
    <header className="flex flex-wrap p-4 w-full relative">
      <div className={`${open ? '!block absolute z-1 right-4 top-14 w-52' : 'hidden sm:block pr-2'} w-2/6 sm:w-fit md:w-1/3 order-2 md:order-1 md:pr-0`}>
      ...
      <div className="w-full sm:flex-1 md:w-1/3 order-1 md:order-2 mb-2 sm:mb-0 flex items-center justify-center sm:justify-start md:justify-center pl-0 sm:pl-2 md:pl-0 h-auto">
        ...
        <div className="w-1/2 sm:hidden flex justify-end">
          <button onClick={() => setOpen(!open)} className="transition-colors duration-200 bg-zinc-800 hover:bg-zinc-800/80 rounded px-1">
            <svg xmlns="http://www.w3.org/2000/svg" width="32" height="32" viewBox="0 0 15 15"><path fill="#eee" fillRule="evenodd" d="M2.5 4a.5.5 0 0 0 0 1h10a.5.5 0 0 0 0-1h-10ZM2 7.5a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 0 1h-10a.5.5 0 0 1-.5-.5Zm0 3a.5.5 0 0 1 .5-.5h10a.5.5 0 0 1 0 1h-10a.5.5 0 0 1-.5-.5Z" clipRule="evenodd"/></svg>
          </button>
        </div>
      </div>

      <div className={`${open ? '!block absolute z-1 right-4 top-24 w-52' : 'hidden sm:flex'} w-4/6 sm:w-fit md:w-1/3 order-3 flex justify-end`}>
